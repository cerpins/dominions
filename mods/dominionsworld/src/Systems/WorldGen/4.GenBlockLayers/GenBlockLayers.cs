        ...
        private void OnChunkColumnGeneration(IServerChunk[] chunks, int chunkX, int chunkZ, ITreeAttribute chunkGenParams = null)
        {
            ...
            for (int x = 0; x < chunksize; x++)
            {
                for (int z = 0; z < chunksize; z++)
                {
                    ...
                    int rand = (int)(distx / 5);
                    float temp = TerraGenConfig.GetScaledAdjustedTemperatureFloat(tempUnscaled, posY - TerraGenConfig.seaLevel + rand);
                    float rainRel = TerraGenConfig.GetRainFall((climate >> 8) & 0xff, posY + rand) / 255f;
